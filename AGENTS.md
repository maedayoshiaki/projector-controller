...
│   ├── EXPERIMENTS.md
│   └── adr/
├── src/projector_controller/      # 実装開始時に追加予定
├── examples/                      # 使用例。必要になったら追加
├── tests/                         # テスト。実装開始時に追加
└── media/                         # 小さなサンプルのみ。大容量素材は置かない
```
...
未確定。最初の Python 実装に入る前に、Python バージョン、依存管理、タスクランナー、GUI / メディア再生バックエンドを人間確認のうえ決める。

### Setup

現時点ではセットアップコマンドはない。`pyproject.toml` などを追加した時点で README とこの節を更新する。
...
現時点では未整備。実装環境ができたら、ここに実コマンドを記載する。
...
- 公開 API は小さく始め、`docs/ARCHITECTURE.md` で候補を比較してから確定する。
