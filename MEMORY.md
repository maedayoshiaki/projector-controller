...
| 2026-06-09 | pygame backend に生ピクセル byte 列表示 `ProjectionWindow.show_frame(data, size, *, pixel_format="RGB", fit_mode=None)` を追加。`pixel_format` は `RGB`(3bpp)/`RGBA`(4bpp) 両対応・既定 RGB。CLI は今回見送り | ユーザー要望「byte 列を渡して画面更新」。既存 `show_image`/`show_test_pattern` と一貫した入口にし、`image.frombuffer`→既存 `_draw_surface`(fit+flip) を再利用。RGB 既定で最小、RGBA も realtime 整合のため対応（人間承認 2026-06-09） | config.py、adapters/{base,pygame_backend}.py、window.py、examples/show_frame.py |
...
- `show_frame` は byte 長（`width*height*bpp`）を **window を開く前に検証**し、不一致は `got/expected` 付き `ValueError` にする。pygame の `image.frombuffer` の不透明な失敗より明確で、GUI なし（backend を open せず）で単体テストできる（`tests/test_show_frame.py`）。`frombuffer` は buffer を非コピーで包むが、`_draw_surface` が同期的に blit/scale して display surface に取り込むため buffer は呼び出し中だけ生きていればよい。`normalize_pixel_format` は `dict[PixelFormat,int]` への `in` で mypy が str→PixelFormat を絞れるため `normalize_fit_mode`(frozenset[str]) と違い `type: ignore` 不要。
- 動画再生の**起動待ち**（黒画面→再生開始）は手元実機で ~704ms あった。内訳計測で `AudioMaster.start()` 内の `sounddevice.query_devices()` が **PortAudio 初期化 ~350ms を同期実行**していたのが最大要因（しかも直後の `RawOutputStream` open でも PortAudio を再初期化＝二重）。**query_devices を外し、デバイス可否は worker の stream open 成否で判定**（失敗時は `play()` が `audio.started()` を見て wall-clock にフォールバック）するだけで ~330ms に半減。残りはデバイス open 自体でハード依存。
