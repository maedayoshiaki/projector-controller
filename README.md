...
`fit_mode` / `--fit-mode` は、**ソース（画像・動画）の縦横比**と**投影ウィンドウの縦横比**が
違うときの収め方を選びます。

| mode | 収め方 | 余白 / はみ出し |
|------|--------|----------------|
| `contain`（既定） | 全体が収まるよう拡縮（縦横比維持） | 余白（レターボックス）が出る |
| `cover` | 画面を埋めるよう拡縮（縦横比維持） | はみ出した端が切れる |
| `stretch` | 画面いっぱいに引き伸ばす（縦横比無視） | 余白なし・歪む |
| `native` | 原寸のまま中央に置く（拡縮なし） | 小さい/大きいまま、中央配置 |

例: 16:9 の画面に正方形画像を出すと、`contain`=左右に余白 / `cover`=上下が切れる /
`stretch`=横に伸びる / `native`=原寸で中央。投影面とソースの比が同じなら 4 つは同じ見た目になります。
