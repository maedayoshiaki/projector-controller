...
- **Status:** `In progress`（方向確定・計測完了。安い改善に着手）
- **Owner:** AI
...
- [~] Rust 受信 `Vec` 再利用 → **保留**。end-to-end 実測で alloc が支配的と分かってから（1080p60 では非支配の見込み）
- [ ] paced producer で renderer 込みの end-to-end を実測（1080p60 を満たすか、遅延が累積しないか）
- [ ] 結果次第で shm 要否を判断（4K60 を本気で狙う段で再評価）
...
- **Status:** `MVP design`（レイヤ=C・デコーダ=PyAV 確定。映像のみ MVP に着手予定。D2 音声・同期は MVP 後）
- **Owner:** AI
- **Created / Updated:** 2026-06-01 / 2026-06-01
...
- [ ] PyAV を `[video]` optional extra として追加（pyproject / uv.lock）
- [ ] renderer 起動ヘルパを切り出し、`RealtimeProjection` をそれに載せ替え（非破壊リファクタ）
- [ ] frame protocol encode を共有可能に
- [ ] `projector_controller.media` プロセス（PyAV デコード→PTS ペース→push）
- [ ] `VideoPlayer` facade（renderer `--backpressure all` + media プロセスの起動・統制）
- [ ] テスト: PyAV で短い合成クリップを生成→ダミー sink に push し受信フレーム数/サイズを検証（GUI 不要）。可能なら renderer 投影 smoke
- [ ] renderer present 遅延を実測し音声オフセットを決定（D2）
- [ ] 音声再生 + A/V 同期を追加（D2）
- [ ] 検証スタック / docs / STATUS / MEMORY
